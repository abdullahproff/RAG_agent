```json
{
  ...
  "tags": ["time_management", "productivity", "planning", "prioritization", "focus", "web_developer", "управление_временем", "продуктивность", "планирование", "приоритизация", "фокус", "веб_разработчик"]
}
```

РОЛЬ: specialist; СПЕЦИАЛЬНОСТЬ: web_developer; ТЕМА: основы_управления_временем_и_продуктивности
...
### Реальность:
Полностью "всё успеть" невозможно. Вместо этого нужно научиться эффективно управлять приоритетами и фокусом в контексте web-разработки.

### Основные проблемы, стоящие за вопросом "как всё успеть":

1. **Отсутствие стратегии и потеря фокуса**
   - Занимаемся тем, чем не надо заниматься (например, излишняя оптимизация UI)
   - Не занимаемся тем, чем надо заниматься (например, написание тестов)

2. **Ощущение перегруженности и FOMO**
   - Fear of Missing Out (страх упустить новые технологии в frontend экосистеме)
...
3. **Непонимание финального результата**
   ...
   - Отсутствие четких критериев выполнения для web проектов

### Ключевой принцип:
**Стратегия — это не тратить время на пустяки в web-разработке**

### Ожидаемый результат:
Использование описанных техник позволит утроить (3x) производительность в повседневной web-разработке.
...
```json
{
  ...
  "specialty": "web_developer",
  "document_type": "time_management_guide",
  "tags": ["strategic_planning", "goal_setting", "eisenhower_matrix", "smart_goals", "web_planning", "стратегическое_планирование", "постановка_целей", "матрица_эйзенхауэра", "smart_цели", "планирование_веб"]
}
```

РОЛЬ: specialist; СПЕЦИАЛЬНОСТЬ: web_developer; ТЕМА: стратегическое_планирование_для_веб_разработчика
...
- **Web-специфика**: Включаем изучение новых технологий, рефакторинг, оптимизацию
...
**Web-примеры приоритизации**:
- **Важно и срочно**: Исправление критических багов в продакшене
- **Важно, но не срочно**: Рефакторинг legacy кода, изучение новых React/Vue версий
...
```json
{
  ...
  "specialty": "web_developer",
  "document_type": "time_management_guide",
  "tags": ["tactical_execution", "task_tracking", "pomodoro_technique", "feedback_loops", "web_development", "тактическое_выполнение", "отслеживание_задач", "техника_помодоро", "обратная_связь", "разработка_веб"]
}
```

РОЛЬ: specialist; СПЕЦИАЛЬНОСТЬ: web_developer; ТЕМА: тактическое_выполнение_задач_разработки
...
- **Web-специфика**: Отдельные колонки для разработки, тестирования, code review
...
- **Web-контекст**: Учитываем зависимости между компонентами и страницами
...
- **Web-специфика**: Code review, unit тесты, cross-browser тестирование
...
```json
{
  ...
  "specialty": "web_developer",
  "document_type": "time_management_guide",
  "tags": ["continuous_process", "regular_review", "productivity_multiplier", "implementation", "web_best_practices", "непрерывный_процесс", "регулярный_пересмотр", "множитель_продуктивности", "внедрение", "лучшие_практики_веб"]
}
```

РОЛЬ: specialist; СПЕЦИАЛЬНОСТЬ: web_developer; ТЕМА: непрерывность_процесса_и_регулярные_пересмотры
...
- **Web-контекст**: Оценка прогресса в изучении технологий, планирование рефакторинга
...
- **Web-специфика**: Анализ качества кода, прогресса в разработке
...
```json
{
  ...
  "specialty": "web_developer",
  "document_type": "time_management_guide",
  "tags": ["additional_resources", "books", "videos", "learning_materials", "web_tools", "дополнительные_ресурсы", "книги", "видео", "учебные_материалы", "инструменты_веб"]
}
```

РОЛЬ: specialist; СПЕЦИАЛЬНОСТЬ: web_developer; ТЕМА: дополнительные_материалы_для_изучения
...
- **Фокус**: Написание читаемого и поддерживаемого JavaScript кода
...
- **Инструменты**: VS Code, WebStorm, Chrome DevTools для повышения продуктивности
- **Техники**: TDD, BDD, Code Review для улучшения качества кода
- **Практики**: Time blocking, Deep work для фокусированной разработки
- **Мониторинг**: Lighthouse, WebPageTest для анализа производительности

### Small reminder:
- **Вопрос**: Как мне всё успевать?
- **Ответ**: Никак.
- **Решение**: Эффективное управление приоритетами и фокусом в web-разработке 